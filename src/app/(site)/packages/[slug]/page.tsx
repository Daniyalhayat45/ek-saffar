...
import { getPackages, getPackageBySlug, getDestinationBySlug } from "@/db/queries";
...
export default async function PackageDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const [destination, allPackages] = await Promise.all([
    getDestinationBySlug(pkg.destinationSlug),
    getPackages(),
  ]);
  const related = allPackages.filter((p) => p.slug !== pkg.slug && p.category === pkg.category).slice(0, 3);
